...
def login_user(request, data: LoginSchema):
    ...
    # Envoi email Mailjet en arriere-plan (evite le timeout)
    print("\n--- ENVOI DU CODE MFA (Background Task) ---")
    message_text = f"Bonjour {user.username},\n\nVotre code de verification est : {code}\n\nCe code est valable 5 minutes.\n\nSi vous n'etes pas a l'origine de cette demande, ignorez cet email."
    
    email_thread = threading.Thread(
        target=send_mail_async,
        args=("Code de verification - Gestion Funeraire", message_text, [user.email])
    )
    email_thread.start()
...
def request_password_reset(request, data: PasswordResetSchema):
    ...
    email_thread = threading.Thread(
        target=send_mail_async,
        args=("Reinitialisation du mot de passe - Gestion Funeraire", message_text, [user.email])
    )
    email_thread.start()
